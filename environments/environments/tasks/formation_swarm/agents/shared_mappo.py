"""Thin MAPPO wrapper that feeds policy observations (not centralized state) to the policy.

SKRL's stock MAPPO passes ``states`` (from ``env.state()``) to both the policy and the
value network. The formation attention encoder needs the 72-dim policy observation.
This subclass overrides ``act`` to inject ``observations`` into the policy's input dict.
"""
