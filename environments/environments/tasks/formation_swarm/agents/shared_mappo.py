"""Paper-aligned MAPPO variant for the formation-swarm task."""
...
class FormationSharedMAPPO(MAPPO):
    """MAPPO with shared actor/critic and observation critic input.

    SKRL's stock MAPPO optimizes each environment agent independently. The Xie et
    al. implementation uses one shared actor, one shared observation critic, and
    batches all drones together during PPO updates. This subclass preserves SKRL's
    rollout/trainer integration but replaces transition value input and update
    logic with the paper's shared-parameter form.
    """
